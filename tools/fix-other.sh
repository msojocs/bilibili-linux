...
notice "屏蔽检测"
grep -lr 'if(!oY' --exclude="app.asar" .
sed -i 's#if(!oY#if(false\&\&!oY#g' "app/main/app.js"
# ==='win';if(!
grep -lr 'if(!Md)' --exclude="app.asar" .
sed -i 's#if(!Md)#if(false\&\&!Md)#' "app/main/app.js"
# global['bootstrapApp']();
grep -lr 'if(oY)' --exclude="app.asar" .
sed -i 's#if(oY)#if(!oY)#' "app/main/app.js"
